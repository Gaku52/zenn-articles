...
![statusline表示例](/images/claude-code-statusline-usage-monitor/statusline-after.png)
*フッター左下に `Ctx: N/A% | 5h: 7% | 7d: 1%` が常時表示される*
...
![Desktop使用量画面](/images/claude-code-statusline-usage-monitor/desktop-usage.png)
*Desktopアプリの設定 → 使用量で確認できる画面。「現在のセッション: 7%」= `5h: 7%`、「週間制限: 1%」= `7d: 1%` と値が一致している*

チラッと見るだけで「まだ余裕がある」「そろそろ控えよう」「`/compact` すべき」が判断できます。

:::message
`/compact` はClaude Codeの組み込みコマンドで、会話履歴をAIが要約・圧縮してコンテキスト使用率を大幅に下げてくれます。会話の文脈は要約として保持されるため、完全にリセットされるわけではありません。
:::
