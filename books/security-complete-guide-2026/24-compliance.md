...
| # | 原則 | 要点 |
|---|------|------|
| 1 | 適法性・公正性・透明性 | 6 つの法的根拠のいずれかに基づく処理、プライバシーポリシーの公開 |
| 2 | 目的の限定 | 収集時に明示された目的のみに使用、目的追加時は互換性テスト |
| 3 | データの最小化 | 必要最小限のデータのみ収集、定期的な必要性レビュー |
| 4 | 正確性 | データを最新・正確に保つ、不正確なデータは遅滞なく訂正 |
| 5 | 保存期間の制限 | 必要な期間のみ保持、保持ポリシーの策定と遵守 |
| 6 | 完全性・機密性 | 暗号化・仮名化・アクセス制御による保護 |
| 7 | アカウンタビリティ | 遵守の証明記録、DPIA の実施、ROPA の維持 |
...
class GDPRCompliance:
    ...
    def data_breach_notification(self, breach_details: dict):
        """データ侵害通知 (第33条/第34条): 72時間以内の監督機関への通知"""
        self.notify_supervisory_authority(breach_details, deadline_hours=72)
        if breach_details['risk_level'] == 'high':
            self.notify_affected_individuals(breach_details)  # 第34条
        self.audit.log("breach_notification", breach=breach_details)
```
...
プライバシーバイデザインの 7 原則 (Ann Cavoukian, 2009):

1. **事後対応ではなく事前防止** — 設計段階からプライバシーリスクを排除
2. **デフォルトでのプライバシー保護** — ユーザが何もしなくても最大限保護
3. **設計への組み込み** — プライバシーはアドオンではなく中核機能
4. **全機能性の確保** — プライバシーとセキュリティの両立
5. **ライフサイクル全体の保護** — 収集から削除まで一貫した保護
6. **可視性と透明性** — 処理の透明性確保、独立した検証可能性
7. **ユーザのプライバシーの尊重** — ユーザ中心のデザイン
...
class DataRetentionManager:
    ...
    def execute_retention_policy(self):
        for table, policy in self.RETENTION_POLICIES.items():
            ...
            expired = self.db.find_expired(table, cutoff)
            getattr(self.db, f"{policy['action']}_records")(table, expired)
```
...
- CC1: 統制環境 / CC2: コミュニケーション / CC3: リスク評価 / CC4: 監視活動
- CC5: 統制活動 / CC6: アクセス制御 / CC7: システム運用 / CC8: 変更管理 / CC9: リスク軽減

**追加カテゴリ (必要に応じて選択)**:
- **A (可用性)**: SLA、バックアップ、DR 計画
- **PI (処理の完全性)**: データ処理の正確性・完全性の保証
- **C (機密性)**: 機密情報の保護、暗号化、アクセス制御
- **P (プライバシー)**: AICPA Privacy Criteria、GDPR との整合
...
| フェーズ | 期間 | 内容 |
|---------|------|------|
| ギャップ分析 | Month 1-2 | TSC マッピング、ギャップ特定、監査法人選定 |
| 統制の実装 | Month 2-4 | ポリシー策定、技術的統制、GRC ツール導入 |
| Type I 監査 (任意) | Month 4-5 | 統制設計の検証、早期問題発見 |
| 観察期間 | Month 5-11 | 最低 6 ヶ月の統制運用、証跡収集 |
| Type II 監査 | Month 11-12 | サンプルテスト、運用有効性検証、レポート発行 |
| 年次更新 | 以降毎年 | Type II レポート更新、継続的な統制運用 |
