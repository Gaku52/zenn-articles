---
title: "状態管理（@State, @Binding, @StateObject等）"
---

# Chapter 02: 状態管理（@State, @Binding, @StateObject等）
...
```swift
struct StateB estPracticesView: View {
    // ✅ privateにする
...
## 2.4 @StateObject と @ObservedObject

参照型（class）の状態管理には、@StateObjectと@ObservedObjectを使用します。

### 2.4.1 @StateObject - View所有のObservableObject
...
## 2.5 @EnvironmentObject - グローバル状態管理
...
### 2.5.3 プレビューでのEnvironmentObject注入
...
## 2.7 よくある間違いとトラブルシューティング

### 2.7.1 間違い1: @StateObjectと@ObservedObjectの混同
...
### 2.7.2 間違い2: @EnvironmentObjectの注入忘れ
...
この章では、SwiftUIの状態管理について学びました：

**学んだこと:**
- @State: View内のローカル状態管理
- @Binding: 双方向データバインディング
- @StateObject/@ObservedObject: 参照型の状態管理
- @EnvironmentObject: グローバル状態の共有
- @Environment: システム環境値へのアクセス
...
適切な状態管理は、保守性が高く、バグの少ないSwiftUIアプリケーションの基盤です。この章で学んだパターンを実際のプロジェクトで活用してください。
